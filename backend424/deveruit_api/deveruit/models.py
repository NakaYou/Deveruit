...
class UserManager(BaseUserManager):
    ...
    def create_user(self, username, github_url, password=None, **extra_fields):

        if not github_url:
            raise ValueError('Users must have an github url')
 
        user = self.model(github_url=github_url, username=username, **extra_fields)
        user.set_password(password)
        user.save(using=self._db)
        return user
 
    def create_superuser(self, email, username, github_url, password=None):
